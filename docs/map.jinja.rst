...
We will see a quick setup to configure the ``directory`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``directory`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``directory`` formulas under ``/srv/salt/directory/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/directory/parameters/dns:domain/
    ln -s dns:domain /srv/salt/directory/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/directory/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/directory/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``directory`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``directory`` formulas under ``/srv/salt/directory/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/directory/parameters/roles
...
We create a configuration for the role ``directory/server`` in ``/srv/salt/directory/parameters/roles/directory/server.yaml``:
...
We create another configuration for the role ``directory/client`` in ``/srv/salt/directory/parameters/roles/directory/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles directory/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles directory/client
...
.. code-block:: console
    ...
    mv /srv/salt/directory/parameters/dns:domain/  '/srv/salt/directory/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-directory-as-json:
      test.nop:
        - name: {{ directory | json }}
...
Here is an example based on `template-formula/directory/config/file.sls`_:
...
This ``sls`` file expose a ``directory`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ directory['winner'] }}
...
.. _template-formula/directory/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/directory/config/file.sls
